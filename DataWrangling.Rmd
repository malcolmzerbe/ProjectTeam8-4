...
Temp_Mem_PGA_Sch_2 <- Mem_PGA_Sch %>%
  select(Player, `Charles Schwab`, Memorial, `PGA Championship`) %>%
...
Temp_Mem_PGA_Sch_2 <- subset(Temp_Mem_PGA_Sch_2, select = -c(playedSchwab, playedMemorial, playedPGA)) # delete extra columns


# add tournplayed variable
=======
Mem_PGA_Sch <-
  Mem_PGA_Sch %>%
  mutate(rank = if_else(grepl("^T",Position), gsub("^T", "",Position), gsub("CUT", "NA", Position))) #add rank variable 

>>>>>>> 82f3158bf9ef7740fde1364f8cf9b310ab47a330
```
...
mutate(tournplayed = if_else(grepl("-[0-9]|[0-9]", `Charles Schwab`), "Played", "Did not play")) %>%
