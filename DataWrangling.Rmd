...
#Top10 variable......WE CAN PUT THIS ALL TOGETHER INTO ONE DATA FRAME IF U GUYS WANT 
Top10 <- 
  Complete_M_P_S %>%
...
MadeCut <-
  Complete_M_P_S%>%
  select(Position,Player, Tournament)%>%
  mutate(MadeCut = if_else(grepl("CUT", Position),"No","Yes"))

#Made Cut joined with top10 
MadeCutJoin <-
  Top10Join%>%
  left_join(MadeCut, by = c("Player", "Position", "Tournament"))

#SalaryRange variable 
SalaryRange <-
  Complete_M_P_S%>%
  select(Player, Salary, Tournament)%>%
  mutate(SalaryRange = case_when(grepl("6[0-9]{3}", Salary) ~ "6000s", grepl("7[0-9]{3}", Salary) ~ "7000s", grepl("8[0-9]{3}", Salary) ~ "8000s",grepl("9[0-9]{3}", Salary) ~ "9000s",grepl("1[0-9]{4}", Salary) ~ "10000s"))
...
#join with SalaryRange 
Join <-
  Join %>%
  left_join(SalaryRange, by = c("Player", "Salary", "Tournament"))
